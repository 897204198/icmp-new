...
import { NavParams, NavController, AlertController, Alert, ModalController, Events } from 'ionic-angular';
 ...
 */
@Component({
  selector: 'page-rfidOpinion',
  templateUrl: 'rfidOpinion.html'
})
export class RfidOpinionPage {
  ...
  submitOpinionHttp(): void {
    if (this.submitValidate()) {
      ...
      this.http.post(submitUtl, params).subscribe((res: Response) => {
        ...
        this.navCtrl.popTo(this.navCtrl.getByIndex(this.navCtrl.length() - 3)).then(() => {
          this.events.publish('rfidStateChange', this.navParams.get('id'), data.inventoryState, data.single_inventory);
        });
      }, (res: Response) => {
